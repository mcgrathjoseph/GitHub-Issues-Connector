...
// orig Pre Joe -----------   
// orig Pre Joe -----------   
// orig Pre Joe -----------   export function initConfig(context: InvocationContext): Config {
// orig Pre Joe -----------   const config = {
// orig Pre Joe -----------   context: context,
// orig Pre Joe -----------   clientId: process.env.AZURE_CLIENT_ID,
// orig Pre Joe -----------   connector: {
// orig Pre Joe -----------   id: `${process.env.CONNECTOR_ID}`,
// orig Pre Joe -----------   name: process.env.CONNECTOR_NAME,
// orig Pre Joe -----------   accessToken: process.env.CONNECTOR_ACCESS_TOKEN,
// orig Pre Joe -----------   description: process.env.CONNECTOR_DESCRIPTION,
// orig Pre Joe -----------   schema: schema as ExternalConnectors.Schema,
// orig Pre Joe -----------   template: template,
// orig Pre Joe -----------   repos: process.env.CONNECTOR_REPOS,
// orig Pre Joe -----------   },
// orig Pre Joe -----------   };
// orig Pre Joe -----------   validateConfig(config);
// orig Pre Joe -----------   context.log("Configuration object initialized");
// orig Pre Joe -----------   
// orig Pre Joe -----------   return config;
// orig Pre Joe -----------   }
// orig Pre Joe -----------   
// orig Pre Joe -----------   
...
// new Joe: PnP-style local / dev detection and conditional validation
// new Joe
export function initConfig(context: InvocationContext): Config {
  // Joe: log mode for clarity
  context.log(
    "Joe initConfig: isDevelopment_Joe =",
    isDevelopment_Joe
  );
  ...
  const config = {
    ...
    // Joe: clientId only required in non-dev mode
  ...
  // Joe: PnP-style skip strict validation in dev
  if (!isDevelopment_Joe) {
    validateConfig(config);
  } else {
    context.log("Joe dev mode: skipping strict config validation (PnP-style)");
  }
 ...
 * 
 * hmm aiTOjoe note
 * maybe not want to disable everything, only identity-related checks.
 * 
 *               if (!isDevelopment_Joe && !config.clientId) { 
 * 
 */
